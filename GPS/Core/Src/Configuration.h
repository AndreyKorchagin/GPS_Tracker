...
void delay(uint32_t time) { for (int i = 0; i < time; i++) { };
}
...
void ConvertToXY(float latitude, float longitude, float* ptrx, float* ptry) {
  ...
  float n = floor(((6 + longitude) /
                   6));  // номер шестиградусной зоны в проекции Гаусса-Крюгера
  float l = (longitude - (3 + 6 * (n - 1))) / 57.2958;
  float l2 = pow(l, 2);
  *ptrx =
      A * latitude -
      sin2latit *
          (16002.89 + 66.9607 * sinlatit2 + 0.3515 * sinlatit4 -
           l2 * (1594561.25 + 5336.535 * sinlatit2 + 26.790 * sinlatit4 +
                 0.149 * sinlatit6 +
                 l2 * (672483.4 - 811219.9 * sinlatit2 + 5420 * sinlatit4 -
                       10.6 * sinlatit6 +
                       l2 * (278194 - 830174 * sinlatit2 + 572434 * sinlatit4 -
                             16010 * sinlatit6 +
                             l2 * (109500 - 574700 * sinlatit2 +
                                   863700 * sinlatit4 - 398600 * sinlatit6)))));

  *ptry = (5 + 10 * n) * 100000 +
          l * cos(latitude) *
              (6378245 + 21346.1415 * sinlatit2 + 107.1590 * sinlatit4 +
               0.5977 * sinlatit6 +
               l2 * (1070204.16 - 2136826.66 * sinlatit2 + 17.98 * sinlatit4 -
                     11.99 * sinlatit6 +
                     l2 * (270806 - 1523417 * sinlatit2 + 1327645 + sinlatit4 -
                           21701 * sinlatit6 +
                           l2 * (79690 - 866190 * sinlatit2 +
                                 1730360 * sinlatit4 - 945360 * sinlatit6))));
}

int DirectionalAngle(float x1, float y1, float x2,
                     float y2) {  // x1, y1 - координаты текущей точки, x2, y2 -
                                  // координаты сохранившейся точки
  float dx, dy, r12;
  dx = x2 - x1;
  dy = y2 - y1;
  int i = 0;
